<div>
    <x-jet-dialog-modal wire:model="isOpen" wire:loading.remove>
        <x-slot name="title">
            ...
            <div class="flex items-start justify-between p-5 rounded-t">
                <h3 class="text-3xl font-semibold">
                    @if ($desig_id)
                        <h1 class="text-gray-700 font-bold">Edit {{ $name }} Designation </h1>
                    @else
                        <h1 class="text-gray-700 font-bold">Create Designation</h1>
                    @endif
                </h3>
        ...
        <x-slot name="content">
            ...
            <div class="mt-2">
                <x-jet-label for="branch" class="font-light text-gray-600" value="{{ __('Department') }}" />
                <select wire:model="department_id" name="department_id"
                    ...
                    <option value="#" class="py-1 text-light text-gray-600">Select a Department</option>
                    @foreach ($departments as $dep)
                        <option class="py-1 text-light text-gray-600" value="{{ $dep->id }}">{{ $dep->name }}
                        </option>
                    @endforeach
                </select>
