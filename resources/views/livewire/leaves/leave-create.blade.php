<div>
    <x-jet-dialog-modal wire:model="isOpen" wire:loading.remove>
        <x-slot name="title">
            ...
            <div class="flex items-start justify-between p-5 rounded-t">
                <h3 class="text-3xl font-semibold">
                    @if ($leave_type_id)
                        <h1 class="text-gray-700 font-bold">Edit {{ $name }} Leave Type </h1>
                    @else
                        <h1 class="text-gray-700 font-bold">Create Leave Type</h1>
                    @endif
                </h3>
        ...
        <x-slot name="content">
            ...
            <div class="mt-4">
                <x-jet-label class="font-light text-gray-600" for="days" value="{{ __('Days') }}" />
                <x-jet-input id="days" type="number"
                    class="w-full mt-2 mb-6 px-6 py-3 border rounded-lg text-lg text-gray-700 focus:outline-none"
                    wire:model.defer="days" />
                @error('days') <span class="error text-red-800 bg-red-50 rounded-md p-2">{{ $message }}</span>
                @enderror
            </div>
        </x-slot>
        <x-slot name="footer">
