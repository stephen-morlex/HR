<x-jet-dialog-modal wire:model="isOpen" maxWidth="2xl">

    <x-slot name="title">
        <h1 class="flex items-center text-light">
            Create Employee
        </h1>
    </x-slot>
    <x-slot name="content">
        <div>
            @if (!empty($successMsg))
                <div class="alert alert-success">
                    {{ $successMsg }}
                </div>
            @endif
            <div class="stepwizard">
                <div class="stepwizard-row setup-panel">
                    <div class="multi-wizard-step">
                        <a href="#step-1" type="button"
                            class="btn {{ $currentStep != 1 ? 'btn-default' : 'btn-primary' }}">1</a>
                        <p>Step 1</p>
                    </div>
                    <div class="multi-wizard-step">
                        <a href="#step-2" type="button"
                            class="btn {{ $currentStep != 2 ? 'btn-default' : 'btn-primary' }}">2</a>
                        <p>Step 2</p>
                    </div>
                    <div class="multi-wizard-step">
                        <a href="#step-3" type="button"
                            class="btn {{ $currentStep != 3 ? 'btn-default' : 'btn-primary' }}"
                            disabled="disabled">3</a>
                        <p>Step 3</p>
                    </div>
                </div>
            </div>

            <div class="row setup-content {{ $currentStep != 1 ? 'display-none' : '' }}" id="step-1">
                <div class="col-md-12">
                    <h3> Step 1</h3>
                    <div class="">
                        <div class=" relative w-full mb-3">
                            <x-jet-label class="font-light text-gray-600" for="name" value="{{ __('Name') }}" />
                            <x-jet-input id="name" type="text"
                                class="w-full mt-2 mb-6 px-6 py-3 border rounded-lg text-lg text-gray-700 focus:outline-none"
                                wire:model.defer="name" />
                            @error('name') <span
                                    class="error text-red-800 bg-red-50 rounded-md p-2">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="relative w-full mb-3">
                            <x-jet-label class="font-light text-gray-600" for="email" value="{{ __('Email') }}" />
                            <x-jet-input id="email" type="email"
                                class="w-full mt-2 mb-6 px-6 py-3 border rounded-lg text-lg text-gray-700 focus:outline-none"
                                wire:model.defer="email" />
                            @error('email') <span
                                    class="error text-red-800 bg-red-50 rounded-md p-2">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class=" flex items-center justify-between">
                            <div class="relative w-full mb-3 mr-2">
                                <x-jet-label class="font-light text-gray-600" for="phone"
                                    value="{{ __('Phone No') }}" />
                                <x-jet-input id="phone" type="number"
                                    class="w-full mt-2 mb-6 px-6 py-3 border rounded-lg text-lg text-gray-700 focus:outline-none"
                                    wire:model.defer="phone" />
                                @error('phone') <span
                                        class="error text-red-800 bg-red-50 rounded-md p-2">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="relative w-full mb-3">
                                <x-jet-label for="gender" class="font-light text-gray-600"
                                    value="{{ __('Gender') }}" />
                                <select wire:model="gender" name="gender"
                                    class="w-full mt-2 mb-6 px-6 py-3 border-gray-400 rounded-lg text-lg text-gray-700  focus:outline-none">
                                    <option class="py-1 text-light text-gray-600" value="0">Male
                                    </option>
                                    <option class="py-1 text-light text-gray-600" value="1">Female
                                    </option>
                                </select>
                                @error('gender') <span
                                        class="error w-full bg-red-50 p-2 rounded-md text-red-800">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
                        <div class="relative w-full mb-3">
                            <div class="relative w-full mb-3 mr-2">
                                <x-jet-label class="font-light text-gray-600" for="address"
                                    value="{{ __('Address') }}" />
                                <x-jet-input id="address" type="textarea"
                                    class="w-full mt-2 mb-6 px-6 py-8 border rounded-lg text-lg text-gray-700 focus:outline-none"
                                    wire:model.defer="address" />
                                @error('address') <span
                                        class="error text-red-800 bg-red-50 rounded-md p-2">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
                        <div class="fixed bottom-0 left-0 right-0 py-5 bg-white shadow-md" x-show="step != 'complete'">
                            <div class="max-w-3xl mx-auto px-4">
                                <div class="flex justify-between">
                                    <div class="w-1/2 text-right">
                                        <button wire:click="firstStep"
                                            class="w-32 focus:outline-none border border-transparent py-2 px-5 rounded-lg shadow-sm text-center text-white bg-blue-500 hover:bg-blue-600 font-medium">Next</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <button class="btn btn-primary nextBtn btn-lg pull-right" wire:click="firstStep"
                        type="button">Next</button>
                </div>
            </div>
            <div class="row setup-content {{ $currentStep != 2 ? 'display-none' : '' }}" id="step-2">
                <div class="col-md-12">
                    <h3> Step 2</h3>
                    <div class="">
                        <div class=" flex items-center justify-between">
                            <div class=" relative w-full mb-3 mr-2">
                                <x-jet-label class="font-light text-gray-600" for="emp_number"
                                    value="{{ __('Employee ID') }}" />
                                <x-jet-input id="emp_number" type="number" placeholder="EMP : " value="EMP"
                                    class="w-full mt-2 mb-6 px-6 py-3 border rounded-lg text-lg text-gray-700 focus:outline-none"
                                    wire:model.defer="emp_number" />
                                @error('emp_number') <span
                                        class="error text-red-800 bg-red-50 rounded-md p-2">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="relative w-full mb-3">
                                <x-jet-label for="branch_id" class="font-light text-gray-600"
                                    value="{{ __('Branch') }}" />
                                <select wire:model="branch_id" name="branch_id"
                                    class="w-full mt-2 mb-6 px-6 py-3 border-gray-400 rounded-lg text-lg text-gray-700  focus:outline-none">
                                    <option class="py-1 text-light text-gray-600">Select
                                        Branch
                                    </option>
                                    @foreach ($branches as $item)
                                        <option class="py-1 text-light text-gray-600" value=" {{ $item->id }}">
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('branch_id') <span
                                        class="error w-full bg-red-50 p-2 rounded-md text-red-800">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
                        <div class=" flex items-center justify-between">
                            <div class="relative w-full mb-3 mr-2">
                                <x-jet-label for="department_id" class="font-light text-gray-600"
                                    value="{{ __('Department') }}" />
                                <select wire:model="department_id" name="department_id"
                                    class="w-full mt-2 mb-6 px-6 py-3 border-gray-400 rounded-lg text-lg text-gray-700  focus:outline-none">
                                    <option class="py-1 text-light text-gray-600" value="1">Select
                                        Department
                                    </option>
                                    @foreach ($departments as $item)
                                        <option class="py-1 text-light text-gray-600" value=" {{ $item->id }}">
                                            {{ $item->name }}
                                        </option>
                                    @endforeach

                                </select>
                                @error('department_id') <span
                                        class="error w-full bg-red-50 p-2 rounded-md text-red-800">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="relative w-full mb-3">
                                <x-jet-label for="designation_id" class="font-light text-gray-600"
                                    value="{{ __('Designation') }}" />
                                <select wire:model="designation_id" name="designation_id"
                                    class="w-full mt-2 mb-6 px-6 py-3 border-gray-400 rounded-lg text-lg text-gray-700  focus:outline-none">

                                    <option class="py-1 text-light text-gray-600" value="1">Select
                                        Designation
                                    </option>
                                    @foreach ($designations as $item)
                                        <option class="py-1 text-light text-gray-600" value=" {{ $item->id }}">
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('designation_id') <span
                                        class="error w-full bg-red-50 p-2 rounded-md text-red-800">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
                        <div class=" flex items-center justify-between">
                            <div class=" relative w-full mb-3 mr-2">
                                <x-jet-label class="font-light text-gray-600" for="join"
                                    value="{{ __('Date of join') }}" />
                                <x-jet-input id="date" type="date"
                                    class="w-full mt-2 mb-6 px-6 py-3 border rounded-lg text-lg text-gray-700 focus:outline-none"
                                    wire:model.defer="join" />
                                @error('join') <span
                                        class="error text-red-800 bg-red-50 rounded-md p-2">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="relative w-full mb-3">
                                <x-jet-label for="status_id" class="font-light text-gray-600"
                                    value="{{ __('Status') }}" />
                                <select wire:model="status_id" name="status_id"
                                    class="w-full mt-2 mb-6 px-6 py-3 border-gray-400 rounded-lg text-lg text-gray-700  focus:outline-none">
                                    <option class="py-1 text-light text-gray-600" value="1">Employee Stattus
                                    </option>
                                    @foreach ($status as $item)
                                        <option class="py-1 text-light text-gray-600" value=" {{ $item->id }}">
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('status_id') <span
                                        class="error w-full bg-red-50 p-2 rounded-md text-red-800">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
                    </div>
                    <div class="w-full text-right flex justify-between items-center">
                        <button wire:click="back(1)"
                            class="w-32 focus:outline-none border border-transparent py-2 px-5 rounded-lg shadow-sm text-center text-white bg-blue-500 hover:bg-blue-600 font-medium">Previous</button>
                        <button wire:click="secondStep()"
                            class="w-32 focus:outline-none border border-transparent py-2 px-5 rounded-lg shadow-sm text-center text-white bg-blue-500 hover:bg-blue-600 font-medium">Next</button>
                    </div>
                </div>
            </div>
            <div class="row setup-content {{ $currentStep != 3 ? 'display-none' : '' }}" id="step-3">
                <div class="col-md-12">
                    <h3> Step 3</h3>
                    <div class="">
                        <div class="relative w-full mb-3">
                            <x-jet-label class="font-light text-gray-600" for="acc_name"
                                value="{{ __('Account Name') }}" />
                            <x-jet-input id="acc_name" type="text"
                                class="w-full mt-2 mb-6 px-6 py-3 border rounded-lg text-lg text-gray-700 focus:outline-none"
                                wire:model.defer="acc_name" />
                            @error('acc_name') <span
                                    class="error text-red-800 bg-red-50 rounded-md p-2">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="relative w-full mb-3 ">
                            <x-jet-label class="font-light text-gray-600" for="acc_number"
                                value="{{ __('Account Number') }}" />
                            <x-jet-input id="acc_no" type="number"
                                class="w-full mt-2 mb-6 px-6 py-3 border rounded-lg text-lg text-gray-700 focus:outline-none"
                                wire:model.defer="acc_no" />
                            @error('acc_no') <span
                                    class="error text-red-800 bg-red-50 rounded-md p-2">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class=" flex items-center justify-between">
                            <div class="relative w-full mb-3 mr-2 ">
                                <x-jet-label class="font-light text-gray-600" for="bank_name"
                                    value="{{ __('Bank Name') }}" />
                                <x-jet-input id="bank_name" type="text"
                                    class="w-full mt-2 mb-6 px-6 py-3 border rounded-lg text-lg text-gray-700 focus:outline-none"
                                    wire:model.defer="bank_name" />
                                @error('bank_name') <span
                                        class="error text-red-800 bg-red-50 rounded-md p-2">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="relative w-full mb-3 ">
                                <x-jet-label class="font-light text-gray-600" for="bank_branch"
                                    value="{{ __('Bank Branch') }}" />
                                <x-jet-input id="bank_branch" type="text"
                                    class="w-full mt-2 mb-6 px-6 py-3 border rounded-lg text-lg text-gray-700 focus:outline-none"
                                    wire:model.defer="bank_branch" />
                                @error('bank_branch') <span
                                        class="error text-red-800 bg-red-50 rounded-md p-2">{{ $message }}</span>
                                @enderror
                            </div>

                        </div>
                        <div class=" flex items-center justify-between">
                            <div class="relative w-full mb-3 mr-2 ">
                                <x-jet-label class="font-light text-gray-600" for="bank_location"
                                    value="{{ __('Bank Location') }}" />
                                <x-jet-input id="bank_location" type="text"
                                    class="w-full mt-2 mb-6 px-6 py-3 border rounded-lg text-lg text-gray-700 focus:outline-none"
                                    wire:model.defer="bank_location" />
                                @error('bank_location') <span
                                        class="error text-red-800 bg-red-50 rounded-md p-2">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="relative w-full mb-3 mr-2 ">
                                <x-jet-label class="font-light text-gray-600" for="tax_payer"
                                    value="{{ __('Tax Payer') }}" />
                                <x-jet-input id="tax_payer" type="number"
                                    class="w-full mt-2 mb-6 px-6 py-3 border rounded-lg text-lg text-gray-700 focus:outline-none"
                                    wire:model.defer="tax_payer" />
                                @error('tax_payer') <span
                                        class="error text-red-800 bg-red-50 rounded-md p-2">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
                        <div class="fixed bottom-0 left-0 right-0 py-5 bg-white shadow-md" x-show="step != 'complete'">
                            <div class="max-w-3xl mx-auto px-4">
                                <div class="flex justify-between">
                                    <div class="w-1/2">
                                        <button
                                            class="w-32 focus:outline-none py-2 px-5 rounded-lg shadow-sm text-center text-gray-600 bg-white hover:bg-gray-100 font-medium border">Previous</button>
                                    </div>

                                    <div class="w-1/2 text-right">
                                        <button wire:click="back(2)"
                                            class="w-32 focus:outline-none border border-transparent py-2 px-5 rounded-lg shadow-sm text-center text-white bg-blue-500 hover:bg-blue-600 font-medium">Previous</button>
                                        <button wire:click="thirdStep()"
                                            class="w-32 focus:outline-none border border-transparent py-2 px-5 rounded-lg shadow-sm text-center text-white bg-blue-500 hover:bg-blue-600 font-medium">Next</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row setup-content {{ $currentStep != 4 ? 'display-none' : '' }}" id="step-4">
                <div class="col-md-12">
                    <h3> Step 4</h3>
                    <div class="bg-white rounded-lg p-10 flex items-center shadow justify-between">
                        <div>
                            <svg class="mb-4 h-20 w-20 text-green-500 mx-auto" viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd"
                                    d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                                    clip-rule="evenodd" />
                            </svg>

                            <h2 class="text-2xl mb-4 text-gray-800 text-center font-bold">Registration Success</h2>
                            <button wire:click="Fouthstep()"
                                class="w-40 block mx-auto focus:outline-none py-2 px-5 rounded-lg shadow-sm text-center text-gray-600 bg-white hover:bg-gray-100 font-medium border">
                                Create Employee
                            </button>
                        </div>
                    </div>
                    <button class="btn btn-success btn-lg pull-right" wire:click="thirdStep"
                        type="button">Create</button>
                    <button class="btn btn-danger nextBtn btn-lg pull-right" type="button"
                        wire:click="back(2)">Back</button>
                </div>
            </div>
        </div>
    </x-slot>
    <x-slot name="footer">

    </x-slot>
</x-jet-dialog-modal>

<script>
    function app() {
        return {
            step: 1,
            passwordStrengthText: '',
            togglePassword: false,

            image: 'data:image/jpeg;base64,',
            password: '',
            branch_id: 'Male',

            checkPasswordStrength() {
                var strongRegex = new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})");
                var mediumRegex = new RegExp(
                    "^(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[A-Z])(?=.*[0-9])))(?=.{6,})");
                let value = this.password;

                if (strongRegex.test(value)) {
                    this.passwordStrengthText = "Strong password";
                } else if (mediumRegex.test(value)) {
                    this.passwordStrengthText = "Could be stronger";
                } else {
                    this.passwordStrengthText = "Too weak";
                }
            }
        }
    }

</script>
<style>
    .display-none {
        display: none;
    }

    .multi-wizard-step p {
        margin-top: 12px;
    }

    .stepwizard-row {
        display: table-row;
    }

    .stepwizard {
        display: table;
        position: relative;
        width: 100%;
    }

    .multi-wizard-step button[disabled] {
        filter: alpha(opacity=100) !important;
        opacity: 1 !important;
    }

    .stepwizard-row:before {
        top: 14px;
        bottom: 0;
        content: " ";
        width: 100%;
        height: 1px;
        z-order: 0;
        position: absolute;
        background-color: #fefefe;
    }

    .multi-wizard-step {
        text-align: center;
        position: relative;
        display: table-cell;
    }

</style>
